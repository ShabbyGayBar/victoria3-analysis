...
class NominalOptimizer:
    ...
    def constraint_urbanization_center(
        self, urbanization_per_center: float = 100.0
    ) -> Self:
        """Append an equality constraint tying Urban Center levels to urbanization.

        Enforces that every *urbanization_per_center* units of urbanization
        generated by buildings corresponds to one level of the Urban Center
        building (``building_urban_center``) via ``A @ x == 0``, where ``A``
        combines the per-configuration ``"urbanization"`` column with the
        Urban Center rows.  Urban Center levels are summed across all of the
        building's production-method configurations.

        Args:
            urbanization_per_center: Units of urbanization per Urban Center
                level.  Defaults to ``100`` (the vanilla rule where each 100
                urbanization in a state creates one Urban Center).
        ...
        A = urbanization - urbanization_per_center * mask
        b = np.array([0.0])
        self.equality_constraints.append((A, b))
        return self
